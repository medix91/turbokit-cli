...
/**
 * Copie favicon.ico depuis templates/ vers le projet
 * @param {string} projectPath 
 */
function copyFavicon(projectPath) {
  const srcFavicon = path.resolve(process.cwd(), 'templates', 'favicon.ico');
  const destFavicon = path.join(projectPath, 'public', 'favicon.ico');

  if (!fs.existsSync(path.dirname(destFavicon))) {
    fs.mkdirSync(path.dirname(destFavicon), { recursive: true });
  }

  fs.copyFileSync(srcFavicon, destFavicon);
  console.log(chalk.gray('✔ favicon.ico copié dans public/'));
}
...
export async function generateProject(answers) {
  ...
  // 8. Copie favicon.ico
  copyFavicon(projectPath);

   // 9. Mise à jour du favicon et du titre dans index.html
  const indexPath = path.join(projectPath, 'index.html');
  if (fs.existsSync(indexPath)) {
    let indexContent = fs.readFileSync(indexPath, 'utf8');

    // Supprime tout <link rel="icon"...> existant
    indexContent = indexContent.replace(
      /<link\s+rel=["']icon["'][^>]*?>/gi,
      ''
    );

    // Ajoute le nouveau favicon juste après <head>
    indexContent = indexContent.replace(
      /<head>/i,
    ...
    console.log(chalk.gray('✔ Mis à jour favicon et titre dans index.html'));
  }


  // 10. Installation des dépendances optionnelles
